...
## Question 1
Generating a series of normal random numbers.  
## Answer
``` {r} 
a=rnorm(10)
a
...
Idea   
Generate U~U(0, 1)     
If F<sub>X</sub> (x<sub>i-1</sub>) < U$\leq$ F<sub>X</sub> (x<sub>i</sub> ), then X = x<sub>i</sub> .    
...
Idea 
Generate random numbers U~U(0, 1) and Y~g(??).  
If U$\leq$(Y ), then accept Y and stop (return X = Y );  
otherwise reject Y and continue.   
In this problem, we choose g(x) =1, 0 < x < 1 and c = $\frac{48}{27}$.
...
Idea:  
First generate 1000 random $??'s$ which has $Gamma(r, ??)$ distribution, then generate $Y's$ which has $Exp(??'s)$ distribution.   

Result 
```{r Code chunk3, echo = FALSE}
n <- 1e3; r <- 4; beta <- 3
...
If $a$ and $b$ are finite, then 
    ...
    where $X\sim U(0,1),g(x)=(b-a)h(x).$  
Use a frequency to approximation the expectation (Strong Law of Large Number): 
    $$\frac1m \sum_{i=1}^m g(X_i),$$
    where $X_1,\ldots,X_m$ are iid copies of $X$ and $m$ is a sufficiently large integer.

Result 
```{r Code chunk4, echo = FALSE}
Pbeta<-function(y){
if(y<=0) value=0
if(y>1) value=1
if(y>=0&y<1){
...
$f(x) = \frac{x}{\sigma^2} e^{-x2/(2??^2)}, x ?? 0, ?? > 0$.   
...
compared with $\frac{X_1+X_2}{2}$ for independent X1, X2?

Result
```{r Code chunk5_1,echo = FALSE}
...
print(sd(MC1))
```
```{r Code chunk5_3,echo = FALSE}
print(sd(MC2))
```
```{r Code chunk5_4,echo = FALSE}
...
Idea:     
$\theta=\int g(x)dx$ can be written as 
    $$\int \frac{g(x)}{f(x)}f(x)dx=E(g(X)/f(X)),$$ 
    where $X$ has pdf/pmf $f(\cdot)$.  
$f(\cdot)$ is a $f(\cdot)$ is a pdf/pmf  from which random samples are easy to generate, which is called the importance function of $g(\cdot)$.    
$\theta$ can be estimated with
    $$\hat\theta=\frac1m\sum_{i=1}^m\frac{g(X_i)}{f(X_i)}$$  
...
From the figure, we can see $f_1$ produces the smaller variance
in estimating.
...
##7.1
Problem statement  
Exercises 7.1 Compute a jackknife estimate of the bias and the standard error of the correlation statistic in Example 7.2. 
...
The 12 observations are the times in hours between failures of airconditioning equipment:
3, 5, 7, 18, 43, 85, 91, 98, 100, 130, 230, 487.
Result  
```{r Code chunk12_1,echo = FALSE}
set.seed(12345)
library(boot)
...
As $\textbf{chisq.test()}$ has a long code, we try a shorter one:
```{r}
...
Check if $\textbf{chisq.text2()}$ returns the same results:
